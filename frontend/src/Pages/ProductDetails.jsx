...
import { IoCheckmarkCircle, IoHeart, IoHeartOutline } from 'react-icons/io5';
...
const ProductDetails = () => {
    ...
    const isLiked = favoriteItems.some((item) => item.id === product.id);
    ...
    return (
        <div className="bg-gray-900 text-gray-200 min-h-screen">
            ...
            <div className="container mx-auto py-8 flex justify-center">
                <div className="relative max-w-3xl w-full bg-gradient-to-r from-gray-800 to-gray-700 p-5 rounded-lg shadow-lg border border-gray-700 flex flex-col lg:flex-row justify-center items-center lg:items-start gap-5">
                    <div className="relative">
                        <img src={product.img} alt={product.title} className="w-full h-72 object-cover rounded mb-5" />
                        <button
                            onClick={toggleFavorite}
                            className="absolute top-3 right-3 bg-gray-800 hover:bg-gray-700 p-2 rounded-full text-white shadow-md"
                        >
                            {isLiked ? <IoHeart size={24} className="text-red-500" /> : <IoHeartOutline size={24} />}
                        </button>
                        <h1 className="text-3xl font-bold text-green-400">{product.title}</h1>
                        <p className="text-md text-gray-300">{product.description}</p>
                        <p className="text-md text-gray-300">Weight : {product.weight}</p>
                    ...
                    <div>
                        ...
                        <form onSubmit={handleReviewSubmit} className="mt-5">
                            ...
                            />
                            {emptyFieldError && (
                                <p className="text-red-500 text-sm mt-2">Please enter both comment and rating.</p>
                            )}
                            <button
                                type="submit"
